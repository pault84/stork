...
var snapshotScheduleColumns = []string{"NAME", "PVC", "POLICYNAME", "PRE-EXEC-RULE", "POST-EXEC-RULE", "RETAIN-POLICY", "LAST-SUCCESS-TIME"}
...
func newCreateSnapshotScheduleCommand(cmdFactory Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	createSnapshotScheduleCommand := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			if len(args) != 1 {
				util.CheckErr(fmt.Errorf("Exactly one name needs to be provided for snapshot schedule name"))
				return
			}
	...
	createSnapshotScheduleCommand.Flags().StringVarP(&pvc, "pvc", "", "", "Name of the PVC for which to create a snapshot schedule")
	...
	createSnapshotScheduleCommand.Flags().BoolVarP(&enabled, "enabled", "", true, "Flag to denote whether schedule should be enabled on creation")
...
func newDeleteSnapshotScheduleCommand(cmdFactory Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	deleteSnapshotScheduleCommand := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(pvc) == 0 {
				if len(args) == 0 {
					util.CheckErr(fmt.Errorf("At least one argument needs to be provided for snapshot schedule name if cluster pair isn't provided"))
					return
				}
