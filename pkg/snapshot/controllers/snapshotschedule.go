...
func (s *SnapshotScheduleController) Handle(ctx context.Context, event sdk.Event) error {
	...
	case *stork_api.VolumeSnapshotSchedule:
		...
		if snapshotSchedule.Spec.Enabled {
...
func getVolumeSnapshotStatus(name string, namespace string) (snapv1.VolumeSnapshotConditionType, error) {
	...
	} else if lastCondition.Type == snapv1.VolumeSnapshotConditionError {
...
func (s *SnapshotScheduleController) updateVolumeSnapshotStatus(snapshotSchedule *stork_api.VolumeSnapshotSchedule) error {
	...
	for _, policyVolumeSnapshot := range snapshotSchedule.Status.Items {
		for _, snapshot := range policyVolumeSnapshot {
			// Get the updated status it we see it as not completed
...
func (s *SnapshotScheduleController) deleteVolumeSnapshots(snapshotSchedule *stork_api.VolumeSnapshotSchedule) error {
	for _, policyVolumeSnapshot := range snapshotSchedule.Status.Items {
		for _, snapshot := range policyVolumeSnapshot {
			err := k8s.Instance().DeleteSnapshot(snapshot.Name, snapshotSchedule.Namespace)
			if err != nil {
				log.VolumeSnapshotScheduleLog(snapshotSchedule).Warnf("Error deleting %v: %v", snapshot.Name, err)
			}
		}
	}
	return nil
